...
def gru_forward(gru, input, lengths, state=None, batch_first=True):
    ...
    packed = torch.nn.utils.rnn.pack_padded_sequence(
        input, input_lengths, batch_first)

    outputs, state = gru(packed, state)
    outputs, output_lengths = torch.nn.utils.rnn.pad_packed_sequence(
        outputs, batch_first=batch_first, total_length=total_length)  # unpack (back to padded)
...
class RepeatNet(nn.Module):
    def __init__(self, embedding_size, hidden_size, item_vocab_size):
        ...
        self.item_emb = nn.Embedding(
            item_vocab_size, embedding_size, padding_idx=0)

        self.enc = nn.GRU(embedding_size, int(hidden_size / 2),
                          num_layers=1, bidirectional=True, batch_first=True)

        self.mode_attn = BilinearAttention(
            hidden_size, hidden_size, hidden_size)
        self.mode = nn.Linear(hidden_size, 2)

        self.repeat_attn = BilinearAttention(
            hidden_size, hidden_size, hidden_size)
        self.explore_attn = BilinearAttention(
            hidden_size, hidden_size, hidden_size)
    ...
    def model(self, data):
        ...
        item_seq_embs = F.dropout(self.item_emb(
            data['item_seq']), p=0.5, training=self.training)

        output, state = gru_forward(
        ...
        explore_feature, attn, norm_attn = self.explore_attn(state.reshape(
            batch_size, -1).unsqueeze(1), output, output, mask=mask.unsqueeze(1))
        p_explore = self.explore(explore_feature.squeeze(1))
        explore_mask = torch.bmm((data['item_seq'] > 0).float(
        ).unsqueeze(1), data['source_map']).squeeze(1)
        # not sure we need to mask this out, depends on experiment results
        p_explore = p_explore.masked_fill(explore_mask.bool(), float('-inf'))
        p_explore = F.softmax(p_explore, dim=-1)

        _, p_repeat = self.repeat_attn.score(state.reshape(
            batch_size, -1).unsqueeze(1), output, mask=mask.unsqueeze(1))
        p_repeat = torch.bmm(p_repeat, data['source_map']).squeeze(1)

        mode_feature, attn, norm_attn = self.mode_attn(state.reshape(
            batch_size, -1).unsqueeze(1), output, output, mask=mask.unsqueeze(1))
        p_mode = F.softmax(self.mode(mode_feature.squeeze(1)), dim=-1)

        p = p_mode[:, 0].unsqueeze(-1)*p_explore + \
            p_mode[:, 1].unsqueeze(-1)*p_repeat
    ...
    def do_train(self, data):
        ...
        loss = F.nll_loss((scores+1e-8).log(),
                          data['item_tgt'].reshape(-1), ignore_index=0)
        return loss
